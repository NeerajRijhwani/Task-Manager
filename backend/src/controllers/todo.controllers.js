...
const AddTask=AsyncHandler(async (req,res)=>{
     ...
     if(req.files && Array.isArray(req.files.taskimage)&& req.files.taskimage.length >0){
...
const UpdateTaskDetails=AsyncHandler(async (req,res)=>{
     ...
     return res.
     status(200).
     json(new ApiResponse(200,UpdateTaskDetails,"Task details Updated successfully"))
})
const UpdateTaskImage=AsyncHandler(async(req,res)=>{
     ...
     if(!deletedTaskimage){
          throw new ApiError(500,"Something went wrong while deleting the previous image")
     }
     ...
     const updatedTodo=await Todo.findByIdAndUpdate(task_id,{
          $set:{
    ...
    .status(200)
    .json(
        new ApiResponse(200, updatedTodo, "Task Image updated successfully")
      )
